...
public void testCurrentTime() {
    ...
    Assert.assertTrue(0 == q.currentTime());
    q.run(expected);
    Assert.assertTrue(expected == q.currentTime());
  }
  ...
  public void testDoActionsAtOrBefore() {
    ...
    _scratch = 0;
    q.run(time1 - 1);
    Assert.assertTrue(0.0 == _scratch);

    _scratch = 0;
    q.run(1);
    Assert.assertTrue(value1 == _scratch);

    q.enqueue(time2, new TestDoActionsAtOrBefore(value2));

    _scratch = 0;
    q.run(time2);
    Assert.assertTrue(value2 == _scratch);
  }
}
